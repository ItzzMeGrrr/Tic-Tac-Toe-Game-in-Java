...
class MultiPlayer {
    ...
    //Enters player in selected space
    static void select_index(int val) 
    {
    ...
    static void Game()// Main game loop
    {
        while (start) {
            draw();
            Input();
    ...
    {
        String again;
        System.out.println("Do you wanna Play it again ?(Y/n)");
        again = sc.next().toString().toLowerCase();
    ...
    static void exit_game()// Exit the Game
    {
        ...
        System.out.println("Enter 1 to Cancel and Anyother key to exit");
        if (sc.next() == "1")
            choices();
        else
